...
// ─── AgencyClient ──────────────────────────────────────────────────────────────
// All interactive UI for the Agency page.
// Initial role/clients/invites are server-fetched and passed as props.
// Mutations (upgrade, invite, delete) still hit the API routes.
// ──────────────────────────────────────────────────────────────────────────────
...
function IconUsers({ className = 'w-5 h-5' }: { className?: string }) {
  return <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6-4a4 4 0 11-8 0 4 4 0 018 0zm6 4a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
}
function IconLink({ className = 'w-5 h-5' }: { className?: string }) {
  return <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
}
...
function IconBolt({ className = 'w-5 h-5' }: { className?: string }) {
  return <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
}
...
function UpgradeBanner() {
  ...
  async function handleUpgrade() {
    ...
    try {
      ...
      if (data.success || data.message) router.refresh()
    } catch {}
    setLoading(false)
  }
...
function InviteModal({ onClose, onCreated }: InviteModalProps) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const data = (await res.json()) as { invite?: AgencyInvite; inviteUrl?: string }
        ...
        {!inviteUrl ? (
          <>
            ...
            <div className="flex gap-3">
              <button onClick={onClose} className="flex-1 px-4 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-xl text-sm transition-colors">Cancel</button>
          ...
          <>
            ...
            <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5 mb-5">
              ...
              <button onClick={copy} className="text-gray-400 hover:text-white transition-colors shrink-0"><IconCopy /></button>
            </div>
            {copied && <p className="text-green-400 text-xs mb-3">Copied to clipboard!</p>}
            <button onClick={onClose} className="w-full px-4 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-xl text-sm transition-colors">Done</button>
          </>
        )}
      </div>
    </div>
...
interface AgencyShellProps {
  ...
  initialInvites: AgencyInvite[]
}

export default function AgencyClientShell({ isAgency, initialClients, initialInvites }: AgencyShellProps) {
      ...
      {!isAgency ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { label: 'Clients',             value: initialClients.length,                                                                        icon: <IconUsers className="w-5 h-5 text-violet-400" /> },
              ...
              { label: 'Total Sites Managed', value: initialClients.reduce((s, c) => s + (c._count?.websites ?? 0), 0),                           icon: <IconBolt  className="w-5 h-5 text-emerald-400" /> },
            ].map((s) => (
              <div key={s.label} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex items-center gap-4">
                <div className="w-10 h-10 bg-gray-800 rounded-xl flex items-center justify-center shrink-0">{s.icon}</div>
                <div>
              ...
              <div className="divide-y divide-gray-800">
                {initialClients.map((c) => (
                  <div key={c.id} className="p-5 flex items-center gap-4">
                    ...
                    <div className="text-gray-600 text-xs shrink-0">{new Date(c.linkedAt).toLocaleDateString()}</div>
                  </div>
                ))}
              </div>
            )}
                ...
                {invites.map((inv) => {
                  ...
                  return (
                    <div key={inv.id} className="p-5 flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="text-gray-300 text-sm truncate font-mono">{`/join?token=${inv.token}`}</div>
