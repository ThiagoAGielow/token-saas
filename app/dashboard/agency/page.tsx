...
function IconUsers({ className = 'w-5 h-5' }: IconProps) {
  return (
...
function IconCopy({ className = 'w-4 h-4' }: IconProps) {
  return (
...
function IconTrash({ className = 'w-4 h-4' }: IconProps) {
  return (
    <svg className={className} width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function IconPlus({ className = 'w-4 h-4' }: IconProps) {
  return (
    <svg className={className} width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function IconBolt({ className = 'w-5 h-5' }: IconProps) {
  return (
    <svg className={className} width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function AgencyPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (roleRes.ok) {
        const { clients: c } = (await roleRes.json()) as { clients?: AgencyClient[] }
        setClients(c ?? [])
        const invRes = await fetch('/api/agency/invite')
        if (invRes.ok) {
          const { invites: inv } = (await invRes.json()) as { invites?: AgencyInvite[] }
          setInvites(inv ?? [])
        }
      }
    } catch {}
