...
// ─── DomainsClient ─────────────────────────────────────────────────────────────
// All interactive logic for the Domains page.
// Initial data is passed from the server component as props.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function DomainsClient({ initialSites, initialDomains }: Props) {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      setShowForm(false); setSubdomain(''); setSiteName(''); setAvailability(null);
  ...
  // ── Custom domain state ──────────────────────────────────────────────────────
  const [customDomains, setCustomDomains]     = useState<CustomDomain[]>(initialDomains);
  ...
  const handleAddDomain = async () => {
    setCdError(null); setCdAdding(true);
    try {
      ...
      setShowCdForm(false); setCdInput('');
    } catch (err) { setCdError((err as Error).message); }
    finally { setCdAdding(false); }
  };
  ...
  const handleVerify = async (id: string) => {
    ...
    try {
      ...
      if (data.verified) setCustomDomains((prev) => prev.map((d) => d.id === id ? { ...d, verified: true } : d));
    } catch { setVerifyResult((prev) => ({ ...prev, [id]: { verified: false, message: 'Check failed' } })); }
  ...
  const handleDeleteDomain = async (id: string) => {
    ...
    } catch { setCdError('Failed to remove domain'); }
  };
  ...
  const isValid = subdomain.trim().length > 0 && SUBDOMAIN_RE.test(subdomain.trim()) && availabilityObj?.available === true;
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            {`${sites.length} site${sites.length !== 1 ? 's' : ''}`} · subdomains on {PLATFORM_DOMAIN}
          </p>
        </div>
      ...
      {showForm && (
        <div className="p-5 rounded-xl bg-[#111] border border-blue-500/20 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-400/10 text-amber-400 border border-amber-400/20">20 tokens</span>
          </div>
          <div>
            ...
            <div className="flex items-center rounded-lg bg-[#1a1a1a] border border-white/10 overflow-hidden focus-within:border-blue-500 transition-colors">
              ...
              />
              <span className="px-3 py-2.5 text-gray-500 text-sm border-l border-white/10 whitespace-nowrap">.{PLATFORM_DOMAIN}</span>
            </div>
            {subdomain && (
              <div className="mt-2 flex items-center gap-1.5 text-xs">
                {availability === 'checking' && <span className="text-gray-500">Checking availability...</span>}
                {availabilityObj?.available === true && <span className="text-green-400 font-medium">{subdomain}.{PLATFORM_DOMAIN} is available</span>}
                {availabilityObj?.available === false && <span className="text-red-400">{availabilityObj.reason || 'Already taken'}</span>}
              </div>
            )}
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => { setShowForm(false); setSubdomain(''); setSiteName(''); setError(null); setAvailability(null); }}
              className="flex-1 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-300 text-sm font-medium transition-colors"
            >Cancel</button>
            <button onClick={handleClaim} disabled={!isValid || claiming}
            ...
            >{claiming ? 'Claiming...' : `Claim ${subdomain || 'Subdomain'} (−20 tokens)`}</button>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl bg-[#111] border border-white/10 overflow-hidden">
        {sites.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-400 text-sm text-center max-w-xs mb-1">Claim a subdomain to get your site live in seconds.</p>
            ...
            <tbody>
              {sites.map((site, i) => (
                <tr key={site.id} className={`${i < sites.length - 1 ? 'border-b border-white/5' : ''} hover:bg-white/2 transition-colors`}>
                  <td className="px-5 py-4"><p className="font-medium text-white">{site.name}</p><p className="text-xs text-gray-500">{site.subdomain}</p></td>
                  ...
                  <td className="px-5 py-4">
                    {site.status === 'ACTIVE'   && <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border bg-green-500/15 text-green-400 border-green-500/25"><span className="w-1.5 h-1.5 rounded-full bg-green-400" />Live</span>}
                    {site.status === 'DRAFT'    && <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border bg-blue-500/15 text-blue-400 border-blue-500/25"><span className="w-1.5 h-1.5 rounded-full bg-blue-400" />Draft</span>}
                    {(site.status === 'BUILDING' || site.status === 'PAUSED') && <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border bg-yellow-500/15 text-yellow-400 border-yellow-500/25"><span className="w-1.5 h-1.5 rounded-full bg-yellow-400" />{site.status === 'PAUSED' ? 'Paused' : 'Building'}</span>}
                  </td>
                  <td className="px-5 py-4 text-right">
                    {deleteConfirm === site.id ? (
                      <div className="flex items-center justify-end gap-1.5">
                        <span className="text-xs text-red-400">Remove?</span>
                        <button onClick={() => handleDelete(site.id)} className="text-xs px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30 transition-colors">Yes</button>
                        <button onClick={() => setDeleteConfirm(null)} className="text-xs px-2.5 py-1 rounded-lg bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10 transition-colors">No</button>
                      </div>
                    ) : (
                      <button onClick={() => setDeleteConfirm(site.id)}
                        className="text-xs px-2.5 py-1 rounded-lg bg-white/5 text-gray-400 border border-white/10 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-all">Remove</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="border-t border-white/10 pt-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { setShowCdForm(!showCdForm); setCdError(null); }}
        ...
        {showCdForm && (
          <div className="p-5 rounded-xl bg-[#111] border border-white/20 space-y-4 mb-4">
            ...
            <div>
              ...
              <input type="text" value={cdInput} onChange={(e) => setCdInput(e.target.value)} placeholder="myshop.com"
                className="w-full px-3 py-2.5 rounded-lg bg-[#1a1a1a] border border-white/10 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors"
              />
            ...
            <div className="flex gap-2">
              <button onClick={() => { setShowCdForm(false); setCdInput(''); setCdError(null); }}
                className="flex-1 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-300 text-sm font-medium transition-colors">Cancel</button>
              <button onClick={handleAddDomain} disabled={!cdInput.trim() || cdAdding}
                className="flex-1 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-400 text-white text-sm font-semibold transition-colors disabled:opacity-40"
              ...
              <div key={d.id} className={`p-4 rounded-xl border ${d.verified ? 'bg-green-500/5 border-green-500/20' : 'bg-[#111] border-white/10'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="p-3 rounded-lg bg-[#1a1a1a] border border-white/10">
                          ...
                          <div className="space-y-1.5 font-mono text-xs">
                            <div className="flex items-center gap-2"><span className="text-gray-500 w-16 flex-shrink-0">Name</span><span className="text-blue-300">_velocitysites-verify.{d.domain}</span></div>
                            <div className="flex items-center gap-2"><span className="text-gray-500 w-16 flex-shrink-0">Type</span><span className="text-gray-300">TXT</span></div>
                            <div className="flex items-center gap-2">
                              ...
                              <button onClick={() => copyToken(d.verificationToken, d.id)}
                        ...
                        <div className="p-3 rounded-lg bg-[#1a1a1a] border border-white/10">
                          ...
                          <div className="space-y-1.5 font-mono text-xs">
                            <div className="flex items-center gap-2"><span className="text-gray-500 w-16 flex-shrink-0">Name</span><span className="text-blue-300">www</span></div>
                            <div className="flex items-center gap-2"><span className="text-gray-500 w-16 flex-shrink-0">Type</span><span className="text-gray-300">CNAME</span></div>
                            <div className="flex items-center gap-2"><span className="text-gray-500 w-16 flex-shrink-0">Value</span><span className="text-amber-300">{PLATFORM_CNAME}</span></div>
                          </div>
                        </div>
                    ...
                    {d.verified && (
                      <a href={`https://www.${d.domain}`} target="_blank" rel="noopener noreferrer"
                        className="text-xs text-green-400 hover:text-green-300 font-mono transition-colors">www.{d.domain} ↗</a>
                    )}
                  </div>
                  <div className="flex-shrink-0">
                    {cdDeleteConfirm === d.id ? (
                      ...
                      <button onClick={() => setCdDeleteConfirm(d.id)}
                        className="text-xs px-2.5 py-1 rounded-lg bg-white/5 text-gray-400 border border-white/10 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-all">Remove</button>
                    )}
                  </div>
                </div>
