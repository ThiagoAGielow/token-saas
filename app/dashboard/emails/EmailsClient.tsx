...
// ─── EmailsClient ─────────────────────────────────────────────────────────────
// All interactive UI for the Emails page.
...
interface DohRecord { data?: string }

function CreateWizard({ domains, balance, onClose, onCreated }: CreateWizardProps) {
  ...
  async function checkMx() {
    ...
    setMxCheck(true);
    try {
      const res  = await fetch(`https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(selectedDomain.domain)}&type=MX`, { headers: { Accept: 'application/dns-json' } });
      const data = (await res.json()) as { Answer?: DohRecord[] };
      setMxOk((data.Answer ?? []).map((r) => r.data?.toLowerCase() ?? '').some((r) => r.includes('zoho.com')));
    } catch { setMxOk(false); }
    finally  { setMxCheck(false); }
  }

  async function handleCreate() {
    setError(null); setLoading(true);
    try {
      const res  = await fetch('/api/emails', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ domainId, username, displayName, password }) });
      const data = (await res.json()) as { email?: EmailAccount; error?: string };
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-lg font-bold text-white">
                {step === 1 ? 'New Mailbox' : step === 2 ? 'Add DNS Records' : 'Creating Mailbox…'}
              </h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center rounded-lg bg-[#1a1a1a] border border-white/10 overflow-hidden focus-within:border-blue-500 transition-colors">
                  <input type="text" value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, ''))} placeholder="hello"
                    className="flex-1 px-3 py-2.5 bg-transparent text-white text-sm placeholder-gray-600 focus:outline-none" />
                  <span className="px-3 py-2.5 text-sm text-gray-500 bg-white/5 border-l border-white/10 whitespace-nowrap">@{selectedDomain?.domain ?? ''}</span>
                </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-amber-400/5 border border-amber-400/20">
                ...
                <p className="text-xs text-gray-400">Add the records below to <span className="text-white font-mono">{selectedDomain?.domain}</span> in your DNS provider.</p>
              </div>
              <div className="rounded-xl border border-white/10 overflow-hidden">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-white/5 border-b border-white/10">
                      {['Type','Host','Value','Priority',''].map((h) => <th key={h} className="text-left px-3 py-2 text-gray-500 font-medium">{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {ZOHO_DNS_RECORDS.map((r, i) => (
                      <tr key={i} className={i < ZOHO_DNS_RECORDS.length - 1 ? 'border-b border-white/5' : ''}>
                        <td className="px-3 py-2.5"><span className={`px-1.5 py-0.5 rounded text-xs font-bold ${r.type === 'MX' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>{r.type}</span></td>
              ...
              <div className="flex items-center justify-between p-3 rounded-lg bg-[#1a1a1a] border border-white/10">
                ...
                {mxOk ? (
                  <span className="flex items-center gap-1.5 text-xs text-green-400 font-semibold">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Verified
                  </span>
                ) : (
              ...
              {error && (
                <div className="space-y-4">
                  <div className="p-4 rounded-xl bg-red-500/5 border border-red-500/20 text-sm text-red-400">
                    <p className="font-semibold mb-1">Something went wrong</p><p>{error}</p>
                  </div>
                  <div className="flex gap-3">
...
interface Props {
  initialEmails:  EmailAccount[];
  initialDomains: CustomDomain[];  // only verified domains
  initialBalance: number;
}

export default function EmailsClient({ initialEmails, initialDomains, initialBalance }: Props) {
  const [emails, setEmails]         = useState<EmailAccount[]>(initialEmails);
  ...
  async function handleDelete(id: string) {
    ...
    } catch { alert('Failed to delete mailbox. Please try again.'); }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{emails.length} mailbox{emails.length !== 1 ? 'es' : ''} · 10 tokens each · Powered by Zoho Mail</p>
        </div>
        <button onClick={() => { if (initialDomains.length === 0) { alert('You need at least one verified domain first. Go to the Domains page.'); return; } setShowCreate(true); }}
      ...
      {initialDomains.length === 0 && (
        <div className="p-4 rounded-xl bg-amber-400/5 border border-amber-400/20 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">You need a verified custom domain to create email accounts.{' '}
              <a href="/dashboard/domains" className="text-blue-400 hover:underline">Verify a domain →</a></p>
          </div>
        </div>
      )}
      ...
      {emails.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[['Mailboxes', emails.length], ['Domains', new Set(emails.map((e) => e.domain?.domain)).size], ['Tokens spent', emails.length * 10]].map(([label, val]) => (
            <div key={String(label)} className="p-4 rounded-xl bg-[#111] border border-white/10">
      ...
      {emails.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-xl bg-[#111] border border-white/10">
          ...
          {initialDomains.length > 0 && (
            <button onClick={() => setShowCreate(true)} className="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-400 text-white text-sm font-semibold transition-colors">Create first mailbox</button>
          )}
        </div>
      ) : (
            ...
            <tbody>
              {emails.map((email, i) => (
                <tr key={email.id} className={`${i < emails.length - 1 ? 'border-b border-white/5' : ''} hover:bg-white/2 transition-colors`}>
                  ...
                  <td className="px-5 py-4 text-gray-400 text-xs">{new Date(email.createdAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
      ...
      <div className="p-4 rounded-xl bg-[#111] border border-white/10 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">Access webmail at <a href="https://mail.zoho.com" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">mail.zoho.com</a> or configure via IMAP/SMTP.</p>
        </div>
      </div>
