  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [emailsData, domainsData, tokensData] = await Promise.all([
        emailsRes.json() as Promise<{ emails?: EmailAccount[] }>,
        domainsRes.json() as Promise<{ domains?: CustomDomain[] }>,
        tokensRes.json() as Promise<{ wallet?: Wallet }>,
      ]);
      setEmails(emailsData.emails ?? []);
      setDomains((domainsData.domains ?? []).filter((d) => d.verified));
      ...
      {emails.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            ['Domains',   new Set(emails.map((e) => e.domain?.domain)).size],
