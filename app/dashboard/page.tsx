...
import type { TokenTransaction, TokensSummary, Website, CustomDomain, EmailAccount } from '@/types/dashboard';

interface IconProps { className?: string }

interface DashboardData {
  tokens: TokensSummary | null;
  websites: { websites?: Website[] } | null;
  domains: { domains?: CustomDomain[] } | null;
  emails: { emails?: EmailAccount[] } | null;
}
...
function txIcon(tx: TokenTransaction) {
...
export default function DashboardPage() {
  const [selectedAction, setSelectedAction] = useState<typeof ACTION_COSTS[number]>(ACTION_COSTS[0]!);
  const [loading, setLoading]               = useState(true);
  const [data, setData]                     = useState<DashboardData | null>(null);

  useEffect(() => {
    Promise.all([
      fetch('/api/tokens').then((r) => r.json() as Promise<TokensSummary>),
      fetch('/api/websites').then((r) => r.json() as Promise<{ websites?: Website[] }>),
      fetch('/api/domains').then((r) => r.json() as Promise<{ domains?: CustomDomain[] }>),
      fetch('/api/emails').then((r) => r.json() as Promise<{ emails?: EmailAccount[] }>),
  ...
  const transactions: TokenTransaction[] = data?.tokens?.transactions ?? [];
