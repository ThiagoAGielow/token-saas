...
type CategoryKey = 'websites' | 'domains' | 'emails' | 'ai' | 'topup';
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#1a1a1a] border border-white/15 rounded-xl p-3 shadow-xl text-xs">
        ...
        <p className="text-gray-400">{Math.round(((item.value ?? 0) / total) * 100)}% of total</p>
      </div>
    );
  }
  return null;
                ...
                {filteredTransactions.map((tx, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-sm">{CATEGORY_ICONS[tx.category as CategoryKey]}</span>
                        <div>
                    ...
                    <td className="px-5 py-3 text-right">
                      <span
                        ...
                        style={{ color: tx.tokens > 0 ? '#22c55e' : CATEGORY_COLORS[tx.category as CategoryKey] || '#ef4444' }}
                      >
