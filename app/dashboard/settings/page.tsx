...
type ProviderColor = 'orange' | 'green' | 'blue';
type Provider = {
  id: string;
  name: string;
  description: string;
...
type ConnectedKey = { provider: string; keyHint: string };
type ColorClasses = { border: string; bg: string; badge: string; btn: string; focus: string; dot: string };
...
const COLOR_MAP: Record<ProviderColor, ColorClasses> = {
  orange: {
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch('/api/ai-keys')
      ...
      .then((data) => {
        ...
        ((data.keys || []) as ConnectedKey[]).forEach((k) => { map[k.provider] = k; });
  ...
  const handleSave = async (provider: string, key: string) => {
    ...
    setConnectedKeys((prev) => ({ ...prev, [provider]: data.key }));
  };
