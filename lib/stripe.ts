 ...
 */
export async function createCheckoutSession(userId: string, priceId: string, mode: 'payment' | 'subscription', email: string): Promise<Stripe.Checkout.Session> {
 ...
 */
export function resolveTokensFromPriceId(priceId: string): { tokens: number; source: 'MONTHLY_PLAN' | 'TOPUP_PACK'; plan?: string } | null {
