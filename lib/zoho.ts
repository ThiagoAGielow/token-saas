...
async function zohoFetch(path: string, options: RequestInit = {}) {
  ...
  const data = await res.json() as { status?: { code?: number; description?: string }; data?: unknown } & Record<string, unknown>
 ...
 */
export async function addDomainToZoho(domain: string) {
 ...
 */
export async function createZohoAccount({ username, domain, displayName, password }: { username: string; domain: string; displayName: string; password: string }): Promise<{ accountId: string; emailAddress: string }> {
  ...
  }) as { accountId?: string; zmId?: string }
 ...
 */
export async function deleteZohoAccount(zohoAccountId: string) {
