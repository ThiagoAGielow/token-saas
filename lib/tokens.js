 ...
 */
export async function grantTokens(userId, amount, source, expiresAt = null, description) {
 ...
 */
export async function spendTokens(userId, amount, description, metadata = {}) {
  if (!Number.isInteger(amount) || amount <= 0) {
 ...
 */
export async function estimateDaysRemaining(userId) {
 ...
 */
export async function expireGrants(userId) {
  const now = new Date()
 ...
 */
export async function grantReferralBonus(referrerId, refereeId) {
  await Promise.all([
    grantTokens(
      referrerId,
      REFERRAL_BONUS,
 ...
 */
function _sourceToTransactionType(source) {
  const map = {
 ...
 * @returns {Promise<void>}
 */
async function _drainGrants(tx, userId, amount) {
  const now = new Date()
