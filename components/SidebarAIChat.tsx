...
function ChatMessage({ msg }: { msg: ChatMessageModel }) {
...
export default function SidebarAIChat() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/websites/${selectedId}/chat`)
      .then((r) => r.json() as Promise<{ messages?: ChatMessageModel[] }>)
  ...
  // ── Send message ─────────────────────────────────────────────────────────────
  async function handleSend(e: FormEvent | KeyboardEvent): Promise<void> {
    e.preventDefault();
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLTextAreaElement>): void {
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') void handleSend(e);
  }
